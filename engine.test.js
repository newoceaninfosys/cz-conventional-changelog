...
describe('commit message', function() {
  it('only header w/ project key', function() {
    expect(
      commitMessage({
        type,
        id: issueKey,
    ...
    ).to.equal(`${type}: NO-${issueKey} ${subject}`);
  });
  it('only header w/ out scope', function() {
    ...
    ).to.equal(`${type}: ${issueKey} ${subject}`);
  });
  it('only header w/ scope', function() {
    ...
    ).to.equal(`${type}(${scope}): ${issueKey} ${subject}`);
  });
  ...
  it('header and body w/ custom scope', function() {
    ...
    ).to.equal(`${type}(${customScope}): ${issueKey} ${subject}\n\n${body}`);
  });
  ...
  it('issueKeyPrepend decorator', function() {
    expect(
      commitMessage(
        {
          type,
  ...
  it('issueKeyAppend decorator', function() {
    expect(
      commitMessage(
        {
          type,
  ...
  it('issueKeyPrepend and issueKeyAppend decorators', function() {
    expect(
      commitMessage(
        {
          type,
...
describe('questions', function() {
  it('default question', function() {
    expect(questionPrompt('id')).to.be.eq('Enter issue id (12345):');
  });
